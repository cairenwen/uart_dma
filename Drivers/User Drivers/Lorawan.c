...
void LoraNode_Init_Mode(LoraNode_Mode_T mode)
{
	...
	Send_AT_Custom("AT+CLASS=2\r\n");
	Send_AT_Custom("AT+CONFIRM=1\r\n");
...
void LoraNode_GetNetMode(Node_Info *LoRa_temp)
{
	...
	if((ptr = StringStr((char *)AT_Data_buf, temp)) != NULL)
	{
		if( *(ptr + 7) == 0x30)
		{
			LoRa_temp->NET_Mode = NET_ABP;
		}
		if(*(ptr + 7) == 0x31)
		{
			LoRa_temp->NET_Mode = NET_OTAA;
		}
	}
...
{
	static char SetRADIO[40] = "AT+RADIO=";
	uint8_t buf[10] = {0}; 
...
{
	static char SetDebug[50] = "AT+RADIO=";
	uint8_t buf[10] = {0}; 
	uint8_t buf1[10] = {0}; 
	uint8_t buf2[10] = {0}; 
	uint8_t buf3[10] = {0}; 
...
uint8_t Send_AT_Custom(char *at_buf)
{
	char *temp = "OK";
	
	LoraNode_Send_AT(at_buf);
...
void LoraNode_Read(uint8_t *str)
{
	...
	while(LPUsart1_RX.receive_flag != 1)
	{
...
int8_t LoRaNode_LowPower_Send(uint8_t *buffer, int32_t len,uint8_t CONFIRM)
{
	...
	if(LORANODE_MODE_STATUS == 0)	
	{
		...
		uint8_t TimeOut_Sign = 0;
		while(HAL_GPIO_ReadPin(LORANODE_BUSY_GPIO_PORT, LORANODE_BUSY_PIN) == GPIO_PIN_RESET)   
		{
			if(Time_Out_Break(2000,&TimeOut_Sign) == 1)
			{
				ret = -1;             // 错误 -1  : 模块没有准备好
		...
		TimeOut_Sign = 0;
		while(HAL_GPIO_ReadPin(LORANODE_BUSY_GPIO_PORT, LORANODE_BUSY_PIN) == GPIO_PIN_SET)
		{
			if(Time_Out_Break(2000,&TimeOut_Sign) == 1)
			{
		...
		if(Send_Sate_Sign == 13)
		{
			...
			if(CONFIRM == 0)
			{
				...
				{
					TimeOut_Sign = 0;
					while(HAL_GPIO_ReadPin(LORANODE_STAT_GPIO_PORT, LORANODE_STAT_PIN) == GPIO_PIN_RESET)      
					{
						if(Time_Out_Break(1000,&TimeOut_Sign) == 1)
						{
...
void LoraNode_STA_Check(Node_Info *LoRa_temp)
{
	...
	if(LoRa_temp->NET_Mode == NET_OTAA)
	{
	...
	}else if(LoRa_temp->NET_Mode == NET_ABP)
		{
			if(LoRa_temp->BUSY == 1)
			{
				LoRa_temp->Join = 1;
	...
	if(LoRa_temp->Join != online_log )
	{
		if(LoRa_temp->Join == 1)
		{
			DEBUG_Printf("--> Network is connected\r\n");
    ...
    if(LoRa_temp.NET_Mode == NET_OTAA)
    {
        ...
        if(Key_show_flag == 1)
        {
          DEBUG_Printf("APPKEY: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x \r\n\r\n",LoRa_temp.AppKEY[0],LoRa_temp.AppKEY[1],LoRa_temp.AppKEY[2],LoRa_temp.AppKEY[3],LoRa_temp.AppKEY[4],LoRa_temp.AppKEY[5],LoRa_temp.AppKEY[6],LoRa_temp.AppKEY[7],
        	...
        	{
          		DEBUG_Printf("APPKEY:The key is not readable!\r\n" );
        	}
                
    }
    else if(LoRa_temp.NET_Mode == NET_ABP)
    {
		DEBUG_Printf("-->Module network mode : ABP \r\n");
        if(Key_show_flag == 1)
        {
          DEBUG_Printf("APPSKEY: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x \r\n",
          ...
          DEBUG_Printf("NWKSKEY: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x \r\n",
        ...
        {
            DEBUG_Printf("APPSKEY:The key is not readable!\r\n" );
            DEBUG_Printf("NWKSKEY:The key is not readable!\r\n" );            
        }
		DEBUG_Printf("DEVADDR: %02x %02x %02x %02x \r\n",LoRa_temp.DevADDR[0],LoRa_temp.DevADDR[1],LoRa_temp.DevADDR[2],LoRa_temp.DevADDR[3]);
